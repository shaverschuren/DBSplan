...
def generate_process_paths(paths, settings):
    ...
    if "niiDir" not in paths:
        paths["niiDir"] = os.path.join(paths["tmpDataDir"], "nifti")
    if "fsDir" not in paths:
        paths["fsDir"] = os.path.join(paths["tmpDataDir"], "freesurfer")
    ...
    if "usedScans" in settings:
        ...
        raise UserWarning("TODO: ScanSelection GUI not yet implemented.")
    ...
    for scanType, pathArray in dcmPaths_dict.items():
        ...
        for subject, path in pathArray.items():
            ...
            if type(path) in [list, str]:
                ...
                nii_path = os.path.join(paths["niiDir"], subject,
                                        scanType + ".nii.gz")
                ...
                if os.path.exists(dcm_path):
                    ...
                    if subject not in paths["dcm_paths"]:
                        paths["dcm_paths"][subject] = {}
                    paths["dcm_paths"][subject][scanType] = dcm_path
                    # Add nii path to paths dict
                    if subject not in paths["nii_paths"]:
...
def dcm2nii(process_paths, paths, settings, verbose=True):
    ...
    # Define iterator
    if verbose:
        iterator = tqdm(range(len(process_paths)), ascii=True,
    ...
    for img_i in iterator:
        ...
        json_exists = os.path.exists(nii_path.replace(".nii.gz", ".json"))
        if (nii_exists and json_exists):
        ...
        if settings["OS"] == "win":
            ...
            raise UserWarning("Operating system not supported "
                              "(or value of settings['OS'] is wrong).")
...
def nii2fs(process_paths, paths, settings, verbose=True):
    ...
    process_paths = [paths for paths in process_paths if paths[2] is not None]
